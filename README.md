...
- Only **additions** are accepted. Modifications and deletions of existing
  ...
  `github-actions[bot]` is exempt — it removes corpus entries that no longer
  add coverage.)
...
Each night, after the PR-processing job has merged any pending submissions,
automation:
...
5. Commits the minimized inputs to a fresh branch and opens a pull request.
   That PR is picked up and merged by the next nightly's PR-processing job.
